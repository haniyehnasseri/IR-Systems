    ...
    public List<WeightedTerm> computeWeights(fbData fbData, Parameters fbParam, Parameters queryParameters) throws Exception {
        try {
            ...
            while (true){
                ...
                for(String s : queryTerms){
                    double zi = (lambda * p_ThetaC.get(s) ) / ((lambda * p_ThetaC.get(s) ) + (1 - lambda) * p_ThetaF.get(s) );
                    p_Zt.put(s, zi);
                }
                double total = 0D;
                for (String s : queryTerms){
                    total += feedback_term_count.get(s) * (1 - p_Zt.get(s));
                }
                for (String s : queryTerms){
                    double newVal = (feedback_term_count.get(s) * (1 - p_Zt.get(s)));
                    double oldVal = p_ThetaF.get(s);
                    if (!df.format(newVal).equals(df.format(oldVal))){
                        p_ThetaF.replace(s, newVal) ;
                        flag = true;
                    }
                }
                if (!flag)
            ...
            for (String s : queryTerms){
                wt.add(new wordWeight(s,p_ThetaF.get(s)));
            }
            return wt;
        }
        catch (Exception e) {
