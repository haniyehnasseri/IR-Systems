...
public class BM25_5 extends ScoringFunctionIterator implements DeltaScoringIterator {
    ...
    public BM25_5(NodeParameters np, LengthsIterator ls, CountIterator it)
            throws IOException {
        super(np, ls, it);
    ...
    private double score(double count, double length) {
        double same = ( (count / (1 - b + (b * length / avgDocLength)) ) + 0.5);
        double numerator = (k + 1) * same;
        double denominator = (k) + same;
